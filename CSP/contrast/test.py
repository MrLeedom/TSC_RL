...
traci.start(sumoCmd)                    #崔西包连接上
tls = traci.trafficlight.getIDList()    #拿到交通信号灯
dets=traci.lanearea.getIDList()         #拿到车道上的探测器id
tls_id = tls[0]
init_p = traci.trafficlight.getPhase(tls_id)  #拿到初始相位
prev = -1
changed = False
ph = [45,25,25,25]
step = 0
...
q_plot_i = []
r_plot_i = []
for _ in range(121):                    #仿真一周期,120s
    traci.simulationStep()
while zq_step < 39: #如果交通网络中的车辆数量大于0
    ...
    if changed:
        if c_p == init_p: #如果当前相位等于初始相位，表示一个周期结束？
            ...
            q_m = sum(queue_sum) / sum(ph)#len(queue_sum)
            r_plot_i.append(r)
            q_plot_i.append(q_m)
            queue_max=[]
            queue_sum = []
