...
from sumtree_buffer import Memory
from sumtree_buffer import Sumtree
...
for i in range(1, num_episodes):
    episodeBuffer0 = Memory()        #首先明确好经验池的大小问题
    ...
    while j < max_epLength:    #一个eposide跑50个周期
        ...
        episodeBuffer0.store(np.reshape(np.array([current_state, action, reward, next_state,done,legal_a_one, legal_a_one_s1]),[1,7])) #Save the experience to our episode buffer.
        ...
        if total_steps > pre_train_steps:
            ...
            if total_steps % (update_freq) == 0:                                             #多少步来更新一次网络
                trainBatch = myBuffer0.sample(batch_size)                          #Get a random batch of experiences.
                ...
                updateTarget(targetOps,sess)                                                #Set the target network to be equal to the primary network.
                myBuffer0.batch_update(indx, err)                                              #更新记忆池
    ...
    myBuffer0.store(episodeBuffer0.buffer)
