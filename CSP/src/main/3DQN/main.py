...
'''
   ...
   Created on Fri Mar 01 18:42:42 2019
   Description:3DQN和常规DQN之间的区别
...
#####################################   3DQN vs  DQN  测试   ######################################################   DQN with priority vs  DQN  测试   ##################################
class DuelingDQN:
    def __init__(
            self,
            n_actions,
    ...
    def _build_net(self):
        def build_layers(s, c_names, n_l1, w_initializer, b_initializer):
            with tf.variable_scope('l1'):  #第一层,两种DQN都一样
            ...
            if self.dueling:
                ...
                with tf.variable_scope('Value'):   #专门分析state的Value
                    ...
                    self.V = tf.matmul(l1, w2) + b2   #一个state一个值,只有一个状态,所以输出一个值

                with tf.variable_scope('Advantage'):  #专门分析每种动作的Advantage
                    w2 = tf.get_variable('w2', [n_l1, self.n_actions], initializer=w_initializer, collections=c_names)
                    b2 = tf.get_variable('b2', [1, self.n_actions], initializer=b_initializer, collections=c_names)
                    self.A = tf.matmul(l1, w2) + b2   #一个动作对应一个值

                with tf.variable_scope('Q'):  #这个是paper中的一种做法,合并V和A,为了不让A直接学成Q,我们减掉A的均值
