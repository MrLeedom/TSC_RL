...
batch_size = 32
anneling_steps = 500. #新设定
num_episodes = 14 #新的设定（上面是原设定）
pre_train_steps = 150 #新的设定（上面是原设定）
...
init_phases = [30,30,30,30]#初始相位时间
...
#     lb='第'+str(i+1)+'轮网络训练'
#     plt.plot(np.arange(len(r_plot[i])), r_plot[i], color=color[i], label=lb)
# plt.legend(loc='upper left')
# plt.ylabel('奖励值')
# plt.xlabel('训练步数')
plt.rcParams['font.sans-serif']=['SimHei']
plt.rcParams['axes.unicode_minus']=False
plt.plot(np.arange(len(r_plot)), r_plot, color='blue')
plt.ylabel('平均奖励值')
plt.xlabel('训练回合数')
plt.show()
saver.save(sess,path+'/model-'+str(i)+'.cptk')
